...
class ParkourDynamic(ParkourEnv):
    ...
    def set_obstacles_new(self):
        ...
        #print("Old positions of obstacles: ", rotating_disc[0], moving_cart[0], stewart_platform[0], stairs[0])
        ...
        stairs[0] = x_coords[2]
        # Set the new positions
        # Print new positions of all obstacles with 3 decimal places
        #print("New positions of obstacles: ", rotating_disc[0], moving_cart[0], stewart_platform[0], stairs[0])
        np.put(self.sim.data.qpos, [24, 25, 26], rotating_disc)
        np.put(self.sim.data.qpos, [32, 33, 34], moving_cart)
        np.put(self.sim.data.qpos, [43, 44, 45], stewart_platform)
        np.put(self.sim.data.qpos, [54, 55, 56], stairs)
